...
class QuipProcessor {
    ...
    async _processThread(quipThread, path) {
        ...
        if(['document', 'spreadsheet'].includes(quipThread.thread.type)) {
            if (this.options.docx && quipThread.thread.type === 'document') {
    ...
    async _processFile(html, file, path, asImage=false) {
        ...
        if(blob) {
            const isValidBlob = checkValidBlob(blob);
            if (isValidBlob) {
                if(asImage) {
                    ...
                    if(file.fileName) {
                        ...
                        const name = file.blobId.trim();
                        const type = Mime.getExtension(blob.type).trim();
                        fileName = `${name}.${type}`;
                    }
