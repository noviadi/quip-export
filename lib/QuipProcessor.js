...
class QuipProcessor {
    ...
    async startExport(folderIds) {
        ...
        const analyzeOnly = !!this.options.analyze;

        await this._exportFolders(folderIdsToExport, analyzeOnly);
        ...
        for(const index in quipFolder.children) {
            ...
            if(quipChild.thread_id) { //thread
                ...
                if (quipChild.restricted) {
                    this.logger.debug("Folder: " + path + " has restricted child: " + quipChild.folder_id);
                } else {
    ...
    async _exportFolders(folderIds, analyzeOnly = false) {
